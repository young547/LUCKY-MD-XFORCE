...
function createNotification(deletedMessage) {
    ...
    let notification = `*[ 💡LUCKY MD XFORCE ANTIDELETE💡 ]*\n\n`;
...
zk.ev.on("messages.upsert", async (m) => {
    if (conf.ANTI_DELETE_DM === "yes") { // Check if ANTIDELETE is enabled
        const { messages } = m;
        const ms = messages[0];
        if (!ms.message) return;
